    ...
    for (l in 1:length(C_list)) {
      ...
      for (i in 1:length(folds)) {
        ...
        svm.Classification.model = svm(Classification ~ Orbit.Axis..AU. + Orbit.Eccentricity + Orbit.Inclination..deg. + Perihelion.Argument..deg. + Node.Longitude..deg. + Mean.Anomoly..deg. + Perihelion.Distance..AU. + Aphelion.Distance..AU. + Orbital.Period..yr. + Minimum.Orbit.Intersection.Distance..AU. + Asteroid.Magnitude,
                                       data=fold.train, kernel=hyper.kernel, cost=hyper.cost, gamma=hyper.gamma, probability = TRUE, type="C-classification" )
      ...
      svm.name <- paste("Clas",hyper.kernel,as.character(hyper.cost),as.character(hyper.gamma.print),sep="_")
      ...
      tdist <- list()
      tdist_name <- paste("Class ",hyper.kernel,as.character(hyper.cost),as.character(hyper.gamma.print),sep=" ")
      ...
      rdist_name <- paste("Class ",hyper.kernel,as.character(hyper.cost),as.character(hyper.gamma.print),sep=" ")  
      rdist <- list()
