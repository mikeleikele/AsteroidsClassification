...
for (j in 1:length(kernel_list)) {
  ...
  if (hyper.kernel == "linear"){
    Gamma_list_select = c('auto')
  }else{
    Gamma_list_select = Gamma_list
  }
  for (g in 1:length(Gamma_list_select)) {
    hyper.gamma.print <- Gamma_list_select[g]
    ...
    for (l in 1:length(C_list)) {
      ...
      for (i in 1:length(folds)) {
        ...
        print(paste(as.character(i), hyper.kernel , as.character(hyper.cost), as.character(hyper.gamma.print), sep = " "))
        svm.Hazardous.model = svm(Hazardous ~ Orbit.Axis..AU. + Orbit.Eccentricity + Orbit.Inclination..deg. + Perihelion.Argument..deg. + Node.Longitude..deg. + Mean.Anomoly..deg. + Perihelion.Distance..AU. + Aphelion.Distance..AU. + Orbital.Period..yr. + Minimum.Orbit.Intersection.Distance..AU. + Asteroid.Magnitude,
          data=fold.train, kernel=hyper.kernel, cost=hyper.cost, gamma=hyper.gamma,type="C-classification", probability = TRUE)
        ...
        spec_false = svm.Hazardous.confusion_matrix_false$byClass["Specificity"]
      ...
      svm.name <- paste("Haz",hyper.kernel,as.character(hyper.cost), as.character(hyper.gamma.print),sep="_")
      ...
      tdist <- list()
      tdist_name <- paste("Haz ",hyper.kernel,as.character(hyper.cost), as.character(hyper.gamma.print),sep=" ")
...
plot.models.color = plot.models.color = c("#E7B800","#E7B800","#E7B800","#E7B800","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#FC4E07","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436","#02b436")
img_name_plot <- paste("IMG_asteroids_model_SVM_", "Hazardous_KFOLD_ROC_FAMILY", ".png", sep = "")
png(img_name_plot,res = 800, height = 10, width = 15, unit='in')
ROCPlot.x.class = colnames(svm.Hazardous_ROC.x)
ROCPlot.y.class = colnames(svm.Hazardous_ROC.y)
plot.new()
...
legend("right", title="models",legend=c('Linear','Polinomial','Radial'),lwd=5,col=c("#E7B800","#FC4E07","#02b436"),horiz=FALSE)
dev.off()
...
img_name_plot <- paste("IMG_asteroids_model_SVM_","Classification_",hyper.kernel,"_C",as.character(hyper.cost), as.character(hyper.gamma.print),"_Amor_Apollo_support", ".png", sep = "")
png(img_name_plot,res = 800, height = 10, width = 15, unit='in')
...
for (j in 1:length(kernel_list)) {
  ...
  if (hyper.kernel != "polynomial"){
    Degree_list_select = c('auto')
  }else{
    Degree_list_select = Degree_list
  }
  
  
  for(d in 1:length(Degree_list_select)) {
    
    for (g in 1:length(Gamma_list_select)) {
      hyper.gamma.print <- Gamma_list_select[g]
      hyper.degree.print <- Degree_list_select[d]
...
img_name_plot <- paste("IMG_asteroids_model_SVM_", "Classification_KFOLD_ROC_FAMILY", ".png", sep = "")
png(img_name_plot,res = 800, height = 10, width = 15, unit='in')
par(mfrow=c(2,2))  
# Amor
plot.new()
...
plot.models.color = c("#000000", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE", "#E60965", "#95CD41", "#FF8E00", "#2596BE")

img_name_plot <- paste("IMG_asteroids_model_SVM_", "Classification_KFOLD_ROC_COST", ".png", sep = "")
png(img_name_plot,res = 800, height = 10, width = 15, unit='in')
par(mfrow=c(2,2))  
# Amor
plot.new()
...
legend("right", title="models",legend=c("Cost 0.5", "Cost 1", "Cost 10", "Cost 100"),lwd=5,col=c("#E60965", "#95CD41", "#FF8E00", "#2596BE"),horiz=FALSE)
#Aten
plot.new()
ROCPlot.x.class = colnames(svm.Classification_ROC.Aten.x)
ROCPlot.y.class = colnames(svm.Classification_ROC.Aten.y)
